...
// ============================================
// MONTH FORMATTING - Compact labels for chart axes
// ============================================
...
/**
 * Convert "April - 2025" to "APR-25" for compact chart labels
 * @param {string} monthStr - e.g. "April - 2025" or "January - 2026"
 * @returns {string} - e.g. "APR-25", "JAN-26"
 */
export const formatShortMonth = (monthStr) => {
    if (!monthStr || typeof monthStr !== 'string') return monthStr;
    const parts = monthStr.trim().split(/\s*-\s*/);
    const monthName = parts[0].toLowerCase().trim();
    const abbrev = MONTH_ABBREVS[monthName] || parts[0].slice(0, 3).toUpperCase();
    if (parts.length >= 2) {
        const year = parseInt(parts[1], 10) || '';
        const shortYear = year ? String(year).slice(-2) : '';
        return shortYear ? `${abbrev}-${shortYear}` : abbrev;
    }
    return abbrev;
};

/**
 * Format an array of month strings for chart labels
 */
export const formatShortMonths = (months) => (Array.isArray(months) ? months : []).map(formatShortMonth);
