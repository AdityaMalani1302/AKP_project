...
const ARAPDashboard = () => {
    ...
    const agingBucketData = useMemo(() => {
        ...
        return {
            ...
            datasets: [{
                ...
                data,
                backgroundColor: [
                    'rgba(52, 211, 153, 0.85)',  // Green - healthy
                    'rgba(251, 191, 36, 0.85)',  // Yellow - warning
                    'rgba(251, 146, 60, 0.85)',  // Orange - concern
                    'rgba(239, 68, 68, 0.85)'   // Red - critical
                ],
                borderColor: [
                    'rgba(16, 185, 129, 1)',
                    'rgba(245, 158, 11, 1)',
        ...
        return {
            ...
            datasets: [{
                ...
                backgroundColor: deviations.map(d => 
                    d > 30 ? 'rgba(239, 68, 68, 0.85)' :
                    d > 0 ? 'rgba(251, 191, 36, 0.85)' :
                    'rgba(52, 211, 153, 0.85)'
                ),
                borderColor: deviations.map(d =>
                    d > 30 ? 'rgba(220, 38, 38, 1)' :
                    d > 0 ? 'rgba(245, 158, 11, 1)' :
                    'rgba(16, 185, 129, 1)'
                ),
                borderWidth: 1,
                borderRadius: 4
    ...
    // Clients Outstanding & Overdue - vertical stacked chart
    // Overdue is part of Outstanding, so we show: Non-Overdue + Overdue = Outstanding
    const clientsBalanceOverdueData = useMemo(() => {
        if (!Array.isArray(rawData) || rawData.length === 0) return null;

        // Get both outstanding and overdue amounts by customer
        const customerData = {};
        ...
        rawData.forEach(row => {
            ...
            if (!customerData[customer]) {
                customerData[customer] = { outstanding: 0, overdue: 0 };
            }
            
            if (desc.includes('overdue amount')) {
                customerData[customer].overdue += amount;
            } else if (desc.includes('total outstanding')) {
                customerData[customer].outstanding += amount;
            }
        });

        // Sort by outstanding and get top 10
        ...
        // Total for datalabels
        const totalData = sorted.map(([, data]) => data.outstanding);
        ...
        return {
            ...
            datasets: [{
                ...
                backgroundColor: growthData.map(g => 
                    g >= 0 ? 'rgba(52, 211, 153, 0.85)' : 'rgba(239, 68, 68, 0.85)'
                ),
                borderColor: growthData.map(g =>
                    g >= 0 ? 'rgba(16, 185, 129, 1)' : 'rgba(220, 38, 38, 1)'
                ),
                borderWidth: 1,
                borderRadius: 4
            }]
    ...
    // Vertical stacked bar options for clients outstanding & overdue with total labels
    const verticalStackedBarOptions = useMemo(() => ({
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
            legend: { position: 'top' },
                ...
                callbacks: {
                    ...
                    afterBody: (tooltipItems) => {
                        if (clientsBalanceOverdueData && clientsBalanceOverdueData.totalData) {
                            ...
                            return `Total Outstanding: ${formatCurrency(clientsBalanceOverdueData.totalData[idx])}`;
                        }
                        return '';
                    }
                }
            ...
            datalabels: {
                display: (context) => context.datasetIndex === 1, // Only show on top dataset (Overdue)
                anchor: 'end',
                align: 'top',
                color: '#1a1a2e',
                font: { weight: 'bold', size: 13 },
    ...
    return (
        <div className="dashboard-container">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <ExportButtons
                        chartRefs={[
                            ...
                            { ref: clientsChartRef, title: 'Clients Balance & Overdue' }
                        ]}
            ...
            <div style={{ marginBottom: '1.5rem' }}>
                {/* Clients Outstanding & Overdue - Vertical Stacked */}
                <div 
                    style={chartCardStyle}
                    onClick={() => clientsBalanceOverdueData && setExpandedChart({
                        title: 'Clients Outstanding & Overdue',
                        content: <Bar ref={clientsChartRef} data={clientsBalanceOverdueData} options={verticalStackedBarOptions} />
                    })}
                >
                    <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ height: '400px' }}>
                        {isLoading ? (
                            ...
                            <Bar ref={clientsChartRef} data={clientsBalanceOverdueData} options={verticalStackedBarOptions} />
                        ) : (
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#6B7280' }}>
                                No data available
                            </div>
