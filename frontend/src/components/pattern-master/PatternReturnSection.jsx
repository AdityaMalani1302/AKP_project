...
const PatternReturnSection = () => {
    ...
    useEffect(() => {
        ...
        const fetchCustomers = async () => {
            try {
                const response = await api.get('/customers', { signal: controller.signal });
                ...
                if (!controller.signal.aborted) console.error('Error fetching customers:', error);
            }
        };
        fetchCustomers();
        return () => controller.abort();
    ...
    const addMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['patternReturnHistory'] });
            handleClear();
        },
        onError: (error) => {
            toast.error(error.response?.data?.error || 'Failed to add return history');
        }
    });
    ...
    const updateMutation = useMutation({
        ...
        onError: (error) => {
            toast.error(error.response?.data?.error || 'Failed to update return history');
        }
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error) => {
            toast.error(error.response?.data?.error || 'Failed to delete return history');
        }
    });
    ...
    return (
        <div className="section-container section-blue" style={{ marginTop: '1rem' }}>
            {/* Header */}
            <h3 style={{ margin: '0 0 1.5rem 0', fontSize: '1.1rem', color: '#0369A1', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <ClipboardList size={20} /> Pattern Return History
            </h3>
                        ...
                        <div className="form-grid" style={{ 
                            ...
                            <div className="form-group" style={{ gridColumn: 'span 1' }}>
                                ...
                                <input
                                    ...
                                    maxLength={500}
                                />
                                <CharacterCounter value={formData.Description} maxLength={500} showAt={400} />
                            </div>
                        </div>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            ...
                            <div>
                                ...
                                />
                                <CharacterCounter value={editFormData.Description} maxLength={500} showAt={400} />
                            </div>
                        </div>
                    ...
                    <div style={{ overflowX: 'auto', border: '1px solid #E5E7EB', borderRadius: '8px', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                            <thead style={{ backgroundColor: '#F9FAFB' }}>
                                <tr>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'center', whiteSpace: 'nowrap', color: '#6B7280' }}>Sr. No</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Pattern No</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Pattern Name</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Customer</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Challan No</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Return Date</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'left', whiteSpace: 'nowrap', color: '#6B7280' }}>Description</th>
                                    <th scope="col" style={{ padding: '0.75rem 1rem', fontWeight: '600', borderBottom: '1px solid #E5E7EB', textAlign: 'center', whiteSpace: 'nowrap', color: '#6B7280' }}>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {returnHistory.length === 0 ? (
                                    ...
                                    returnHistory.map((record, index) => (
                                        <tr key={record.ReturnId} style={{ borderBottom: '1px solid #E5E7EB', transition: 'background-color 0.15s' }}
