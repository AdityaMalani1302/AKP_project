...
const SleeveIndent = () => {
    ...
    const aggregatedSleeves = useMemo(() => {
        const sleeveAggregation = {};
        sleeveIndentData.forEach(row => {
            const details = row.SleeveDetails || [];
            details.forEach(d => {
                if (!sleeveAggregation[d.name]) sleeveAggregation[d.name] = 0;
                sleeveAggregation[d.name] += d.requiredQty || 0;
            });
        });
        return Object.entries(sleeveAggregation).map(([name, qty]) => ({ name, qty }));
    }, [sleeveIndentData]);
                                    ...
                                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        ...
                                        <tbody>
                                            ...
                                            <tr style={{ background: '#fef3c7' }}>
                                                ...
                                                <td style={{ padding: '12px 20px', textAlign: 'center', fontWeight: '700', fontSize: '14px', borderTop: '2px solid #1e3a5f' }}>{aggregatedSleeves.reduce((sum, s) => sum + s.qty, 0)}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
