...
const CORE_COLUMNS = [
    ...
    { key: 'CoreType', label: 'Core Type', align: 'left', width: '130px', sum: false },
    { key: 'TotalNoOfCore', label: 'Total No of Core', align: 'right', width: '150px', sum: true },
    { key: 'ShellQty', label: 'Shell Qty', align: 'right', width: '120px', sum: true },
    { key: 'ColdBoxQty', label: 'Cold Box Qty', align: 'right', width: '130px', sum: true },
    { key: 'NoBakeQty', label: 'No Bake Qty', align: 'right', width: '130px', sum: true },
    { key: 'ProductionQty', label: 'Production Qty', align: 'right', width: '140px', sum: true },
    { key: 'CoreWeight', label: 'Core Wt (kg)', align: 'right', width: '130px', sum: false },
    { key: 'TotalCoreWeight', label: 'Total Core Weight (kg)', align: 'right', width: '190px', sum: true },
];

const COL_COUNT = CORE_COLUMNS.length + 1;
...
const CoreCalculationReport = () => {
    ...
    const { data: records = [], isLoading, isError } = useQuery({
        queryKey: ['core-calculation-report', filters.fromDate, filters.toDate, filters.patternNo],
        queryFn: async () => {
            ...
            const res = await api.get(`/planning/core-calculation?${params.toString()}`);
            return res.data;
        },
    ...
    useEffect(() => {
        if (isError && submitted) {
            toast.error('Failed to load core calculation report');
        }
    }, [isError, submitted]);
    ...
    const handleExport = useCallback(async () => {
        ...
        const worksheet = workbook.addWorksheet('Core Calculation Report');
        ...
        const filename = `Core_Calculation_Report_${filters.fromDate}_to_${filters.toDate}.xlsx`;
    ...
    return (
        <>
            <div className="section-container section-blue" style={{ marginBottom: '1.5rem' }}>
                <h3 className="section-title blue">Core Calculation Report</h3>
                            ...
                            {submitted && records.length > 0 && (
                                <tfoot>
                                    <tr style={{ borderTop: '2px solid #6B7280' }}>
                                        <td style={{ ...cellStyle('center', false), fontWeight: '700', backgroundColor: '#ECFDF5', borderTop: '2px solid #6B7280' }}>Total</td>
                                        {CORE_COLUMNS.map((col) => (
                                            <td
                                                key={col.key}
                                                style={{
...
export default CoreCalculationReport;
