...
const SLEEVE_COLUMNS = [
    ...
    { key: 'SleeveType', label: 'Sleeve Type', align: 'left', width: '150px', sum: false },
    { key: 'SleeveSize', label: 'Sleeve Size', align: 'left', width: '130px', sum: false },
    { key: 'SleeveCount', label: 'No of Sleeves', align: 'right', width: '120px', sum: false },
    { key: 'TotalSleeves', label: 'Total Sleeves', align: 'right', width: '140px', sum: true },
];

const COL_COUNT = SLEEVE_COLUMNS.length + 1;
...
const SleeveCalculationReport = () => {
    ...
    const { data: records = [], isLoading, isError } = useQuery({
        queryKey: ['sleeve-calculation-report', filters.fromDate, filters.toDate, filters.patternNo],
        queryFn: async () => {
            ...
            const res = await api.get(`/planning/sleeve-calculation?${params.toString()}`);
            return res.data;
        },
    ...
    useEffect(() => {
        if (isError && submitted) {
            toast.error('Failed to load sleeve calculation report');
        }
    }, [isError, submitted]);
    ...
    const handleExport = useCallback(async () => {
        ...
        const worksheet = workbook.addWorksheet('Sleeve Calculation Report');
        ...
        const filename = `Sleeve_Calculation_Report_${filters.fromDate}_to_${filters.toDate}.xlsx`;
    ...
    return (
        <>
            <div className="section-container section-blue" style={{ marginBottom: '1.5rem' }}>
                <h3 className="section-title blue">Sleeve Calculation Report</h3>
                            ...
                            {submitted && records.length > 0 && (
                                <tfoot>
                                    <tr style={{ borderTop: '2px solid #6B7280' }}>
                                        <td style={{ ...cellStyle('center', false), fontWeight: '700', backgroundColor: '#F5F3FF', borderTop: '2px solid #6B7280' }}>Total</td>
                                        {SLEEVE_COLUMNS.map((col) => (
                                            <td
                                                key={col.key}
                                                style={{
...
export default SleeveCalculationReport;
