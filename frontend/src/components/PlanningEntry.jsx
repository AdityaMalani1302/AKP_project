...
const PlanningEntry = () => {
    ...
    const filteredRecords = useMemo(() => {
        return existingRecords.filter(record => {
            ...
            if (filterDate) {
                const recordDate = String(record.PlanDate).substring(0, 10);
                matchDate = recordDate === filterDate;
            }
            if (filterShift) {
                matchShift = String(record.Shift) === filterShift;
            }
            return matchDate && matchShift;
        });
    }, [existingRecords, filterDate, filterShift]);
    ...
    const {
        ...
        changePageSize,
        resetToFirstPage
    } = usePagination({ data: filteredRecords, pageSize: 50, autoPaginate: true });

    const { sortKey, sortOrder, handleSort } = useSortableData(filteredRecords);

    const { toggleRow, toggleAll, clearSelection, isSelected, isAllSelected, isSomeSelected, selectedCount, getSelectedRecords } = useRowSelection({ data: filteredRecords, idField: 'EntryId' });

    const displayData = useMemo(() => {
        if (!sortKey || !filteredRecords.length) {
    ...
    }, [filteredRecords, sortKey, sortOrder, currentPage, pageSize]);

    useEffect(() => {
        resetToFirstPage();
    }, [filterDate, filterShift, resetToFirstPage]);
    ...
    useEffect(() => {
        ...
        if (selectedPattern) {
            ...
            fetchPartsForPattern(selectedPattern.PatternId, controller.signal);
            fetchPatternDetails(selectedPattern.PatternId, controller.signal);
        } else {
            setSelectedParts([]);
    ...
    const fetchPartsForPattern = async (patternId, signal) => {
        setLoadingParts(true);
        try {
            const response = await api.get(`/pattern-master/parts-by-pattern/${patternId}`, { signal });
            if (!signal?.aborted) setSelectedParts(response.data);
        } catch (err) {
    ...
    const fetchPatternDetails = async (patternId, signal) => {
        try {
            const response = await api.get(`/pattern-master/${patternId}`, { signal });
            if (!signal?.aborted) setPatternDetails(response.data);
        } catch (err) {
            if (err.name === 'CanceledError' || err.code === 'ERR_CANCELED') return;
    ...
    const handleAddToCalculatedTable = () => {
        ...
        const plateQtyNum = parseInt(plateQty);
        if (String(plateQtyNum) !== String(plateQty).trim()) {
            toast.error('Plate Qty must be a whole number');
            return;
        }
        if (!plateQty || Number.isNaN(plateQtyNum) || plateQtyNum <= 0) {
        ...
        // Part numbers as comma-separated list (store ProdId, not InternalPartNo string)
        const partNoList = selectedParts.map(part => part.PartNo).join(', ');
        ...
        const sleeve = (() => {
            ...
            return sleeves
                .filter(s => s.sleeve_type_size)
                .map(s => `${s.sleeve_type_size_name || s.sleeve_type_size}=${s.quantity || 1}`)
                .join(', ') || '-';
        })();
        ...
        const newEntry = {
            ...
            parts: selectedParts.map(p => ({
                partNo: p.PartNo, // Store ProdId, not InternalPartNo string
                partName: p.PartName || '-',
    ...
    const handleSubmitAll = async () => {
        ...
        try {
            await api.post('/planning-entry', { entries: stagedEntries });
    ...
    const handlePreviewAndPrint = async () => {
        ...
        try {
            ...
            const filtered = existingRecords.filter(record => {
                ...
                const shiftMatch = String(record.Shift) === printShift;
    ...
    const handleBulkDelete = async () => {
        ...
        const results = await Promise.allSettled(selected.map(r => api.delete(`/planning-entry/${r.EntryId}`)));
    ...
    const handleEditRecord = (record) => {
        setEditingRecord({
            ...record,
            planDate: record.PlanDate ? String(record.PlanDate).substring(0, 10) : '',
    ...
    const handleUpdateRecord = async () => {
        ...
        if (!editingRecord.planDate) {
            toast.error('Plan Date is required');
            return;
        }
        const editPlateQty = parseInt(editingRecord.plateQty);
        if (!editingRecord.plateQty || isNaN(editPlateQty) || editPlateQty <= 0) {
            toast.error('Please enter a valid Plate Qty');
            return;
        }
        if (!editingRecord.shift) {
            toast.error('Please select a Shift');
            return;
        }
        ...
        <div>
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        <Combobox
                            ...
                            placeholder={loadingPatterns ? 'Loading patterns...' : 'Select or type Pattern No...'}
            ...
            {selectedParts.length > 0 && (
                <div className="section-container section-purple" style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ marginTop: '1rem' }}>
                        <button onClick={handleAddToCalculatedTable} className="btn btn-primary" disabled={!patternDetails}>
                            {!patternDetails ? 'Loading Pattern Details...' : 'OK - Calculate & Add'}
                        </button>
                    </div>
                </div>
            )}
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                            <thead style={{ position: 'sticky', top: 0, backgroundColor: '#F9FAFB', zIndex: 1 }}>
                                <tr>
                                    ...
                                    <SortableHeader columnKey="PlanDate" label="Plan Date" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="CustomerName" label="Cust_Name" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="PartNo" label="Part No" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="PartName" label="Part Name" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="Cavity" label="Cavity" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="Weight" label="Weight (kg)" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="CoreType" label="Core" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="PlateQty" label="Plate Qty" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={{ ...thStyle, backgroundColor: '#FEF08A' }} />
                                    <SortableHeader columnKey="ProductionQty" label="Production Qty" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={{ ...thStyle, backgroundColor: '#FEF08A' }} />
                                    <SortableHeader columnKey="CastWeight" label="Cast. Wt" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="TotalWeight" label="Total Wt." sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="BoxesPerHeat" label="Boxes/Heat" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="NoOfHeats" label="No of Heats" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="Sleeves" label="Sleeves" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="Shift" label="Shift" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                                    <SortableHeader columnKey="MouldBoxSize" label="Mould Box" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={thStyle} />
                            ...
                            <tbody>
                                {displayData.map((record, index) => {
                                    ...
                                    return Array.from({ length: rowCount }).map((_, partIndex) => {
                                        ...
                                        return (
                                            <tr key={`${record.EntryId}-${partIndex}`} style={{ backgroundColor: isSelected(record.EntryId) ? '#FEF3C7' : (index % 2 === 0 ? 'white' : '#F9FAFB') }}>
                                                {/* Checkbox - rowSpan on first row */}
                                                {isFirstRow && (
                                                    <td rowSpan={rowCount} style={{ ...tdStyle, textAlign: 'center', verticalAlign: 'middle' }} onClick={(e) => e.stopPropagation()}>
                                                        <input
                                                            type="checkbox"
                                                            checked={isSelected(record.EntryId)}
                                                            onChange={() => toggleRow(record.EntryId)}
            ...
            {editingRecord && (
                <div
                    role="dialog"
                    aria-modal="true"
                    aria-label="Edit Planning Entry"
                    onKeyDown={(e) => { if (e.key === 'Escape') setEditingRecord(null); }}
                    style={{
