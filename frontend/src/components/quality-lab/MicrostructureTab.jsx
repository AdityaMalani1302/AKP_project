...
const MicrostructureTab = () => {
    ...
    const { data: records = [], isLoading, isError } = useQuery({
        queryKey: ['microstructure', debouncedSearchTerm],
        queryFn: async () => {
            const url = debouncedSearchTerm ? `/quality-lab/microstructure?search=${encodeURIComponent(debouncedSearchTerm)}` : '/quality-lab/microstructure';
        ...
        <>
            <div style={sectionGreen}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                    <button onClick={handleAdd} className="btn btn-primary btn-ripple">ADD</button>
                    <button onClick={handleEdit} className="btn btn-success btn-ripple">EDIT</button>
