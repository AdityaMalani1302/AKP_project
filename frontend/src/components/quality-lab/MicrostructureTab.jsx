...
const MicrostructureTab = () => {
    ...
    const optimistic = withOptimisticUpdate(queryClient, ['microstructure']);

    const addMutation = useMutation(optimistic.add({
        apiFn: (data) => api.post('/quality-lab/microstructure', data),
    ...
    const updateMutation = useMutation(optimistic.update({
        apiFn: ({ id, data }) => api.put(`/quality-lab/microstructure/${id}`, data),
    ...
    const deleteMutation = useMutation(optimistic.remove({
        apiFn: (id) => api.delete(`/quality-lab/microstructure/${id}`),
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            await Promise.all(selected.map(r => api.delete(`/quality-lab/microstructure/${r.ID}`)));
        ...
        <>
            ...
            <div style={sectionGray}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    ...
                    Microstructure & Hardness Records ({totalRecords})
                    </h3>
                    {selectedCount > 0 && (
                            ...
                            <table style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                    <tr>
                                        ...
                                        <SortableHeader columnKey="Nodularity" label="Nodularity" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="Graphitetype" label="Graphite Type" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="NodularityCount" label="Nodularity Count" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="GraphiteSize" label="Graphite Size" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="Pearlite" label="Pearlite" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="Ferrite" label="Ferrite" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="Carbide" label="Carbide" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                        <SortableHeader columnKey="CastingHardness" label="Hardness" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} style={tableHeaderStyle} />
                                    </tr>
                                </thead>
                                <tbody>
                                    {displayData.length === 0 ? (
                                        <tr><td colSpan={14} style={{ textAlign: 'center', padding: '2rem', color: '#9CA3AF' }}>No records found</td></tr>
                                    ) : displayData.map((record, index) => (
                                        <tr key={record.ID} onClick={() => handleRowClick(record)}
                                            style={{ cursor: 'pointer', backgroundColor: isSelected(record.ID) ? '#FEF3C7' : selectedId === record.ID ? '#DCFCE7' : 'white', transition: 'background-color 0.15s' }}
