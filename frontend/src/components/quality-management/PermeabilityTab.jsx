...
const PermeabilityTab = () => {
    ...
    const { data: sandRecords = [], isLoading } = useQuery({
        queryKey: ['sandProperties-chart', 'permeability', formData.startDate, formData.endDate],
        queryFn: async () => {
    ...
    const chartData = useMemo(() => {
        ...
        return sorted.map(r => parseFloat(r['Permeability In No'])).filter(v => !isNaN(v) && v > 0);
    }, [sandRecords]);

    const labels = chartData.map((_, i) => `R${i + 1}`);
    const USL = 160, LSL = 110, UWL = 150, LWL = 120;

    const data = {
        labels,
        datasets: [{
            label: 'Permeability No', data: chartData,
    ...
    const options = {
        ...
        scales: {
            ...
            y: { min: 110, max: 160, ticks: { stepSize: 10, font: { size: 11 } }, grid: { color: '#E5E7EB', drawBorder: false } }
        },
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: '1rem', fontSize: '0.875rem', color: '#374151' }}><strong>Characteristics:</strong> Permeability No | <strong>Specification:</strong> 110 TO 160</div>
                ...
                <div style={{ textAlign: 'center', padding: '3rem', color: '#9CA3AF', backgroundColor: '#F9FAFB', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
                    ...
                    <p style={{ fontSize: '0.875rem' }}>
                        {formData.startDate && formData.endDate
                            ? 'No Permeability records found for the selected date range.'
            ...
            {!isLoading && chartData.length > 0 && (
                <>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', marginTop: '1rem', fontSize: '0.875rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><div style={{ width: 16, height: 16, backgroundColor: '#22C55E', borderRadius: 3 }} /><span>In Specification (120-150)</span></div>
...
export default PermeabilityTab;
