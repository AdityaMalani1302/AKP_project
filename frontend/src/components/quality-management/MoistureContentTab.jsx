...
const MoistureContentTab = () => {
    ...
    const { data: sandRecords = [], isLoading } = useQuery({
        queryKey: ['sandProperties-chart', 'moisture', formData.startDate, formData.endDate],
        queryFn: async () => {
    ...
    // Extract Moisture values from records (sorted by Date ASC)
    const chartData = useMemo(() => {
        const sorted = [...sandRecords].sort((a, b) => new Date(a.Date) - new Date(b.Date));
        return sorted
            .map(r => parseFloat(r['Moisture In %']))
        ...
        datasets: [
            {
                ...
                pointBackgroundColor: chartData.map(val => {
                    if (val < 3.5 || val > 4.5) return '#EF4444'; // Red - out of spec
                    if (val < 3.75 || val > 4.25) return '#F59E0B'; // Yellow - warning
                    return '#22C55E'; // Green - in spec
                }),
                pointBorderColor: chartData.map(val => {
                    if (val < 3.5 || val > 4.5) return '#DC2626';
                    if (val < 3.75 || val > 4.25) return '#D97706';
                    return '#16A34A';
                }),
                pointRadius: 4,
                pointHoverRadius: 6,
            ...
            tooltip: {
                callbacks: {
                    label: function(context) {
                        return `Reading ${context.label}: ${context.parsed.y}%`;
                    }
                }
            }
        },
    ...
    const calculateStats = () => {
        if (chartData.length === 0) {
            return { average: '0', stdDev: '0', cp: '0', cpk: '0', sigma3: '0', sigma6: '0', upperLimit: '4.500', lowerLimit: '3.500' };
        }
        ...
        // Specification limits for Moisture Content
        const upperLimit = 4.5;
        const lowerLimit = 3.5;
    ...
    return (
        <div>
            ...
            <div style={{
                ...
                <strong>Characteristics:</strong> Moisture % | <strong>Specification:</strong> 3.5% TO 4.5%
            </div>
                ...
                <div style={{ textAlign: 'center', padding: '3rem', color: '#9CA3AF', backgroundColor: '#F9FAFB', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
                    ...
                    <p style={{ fontSize: '0.875rem' }}>
                        {formData.startDate && formData.endDate
                            ? 'No Moisture Content records found for the selected date range.'
                            : 'Select start and end dates to load the chart.'}
                    </p>
                </div>
            )}
                ...
                <>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <span>In Specification (3.75% - 4.25%)</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
...
export default MoistureContentTab;
