...
/****** Object:  View [dbo].[RejectionAnalysis]    Script Date: 03/27/2026 15:24:51 ******/
SET ANSI_NULLS ON
GO
...
ALTER view [dbo].[RejectionAnalysis] as (
select cast(datename(month, Dates)as varchar)+'-' +cast(datename(year,dates)as varchar) as Months,isnull(Sum(OkQTy),0) ProductionQty,isnull(Sum(OkQty*ir.RawCastingWeight),0) as ProductionWeight,isnull(sum(OutQty),0) SubconOutQty,round(isnull(sum(OutQty*ir.RawCastingWeight),0),2) as SubconOutweight
