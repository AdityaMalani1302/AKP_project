...
$filesToDelete = @(
    ...
    # -- Backend Migration Scripts (ALREADY DELETED on 2026-01-12) --
    # These one-time migration scripts were deleted as they had already been executed:
    # SQL Scripts:
    #   - add_additional_info_columns.sql, add_allowed_pages_column.sql
    #   - add_cascade_constraints.sql, add_box_per_heat_column.sql
    #   - alter_pattern_tables.sql, add_nickel_moly_columns.sql
    #   - add_pattern_received_date_column.sql, add_new_columns.sql
    #   - add_moulding_columns.sql, add_main_side_loose_core_qty_columns.sql
    #   - add_core_type_columns.sql, add_core_quantity_columns.sql
    #   - create_performance_indexes.sql, create_lab_master_table.sql
    #   - create_drawing_master_table.sql, create_core_details_master_table.sql
    #   - create_sleeve_master_table.sql, reorganize_pattern_master_table.sql
    #   - rename_actual_casting_weight_column.sql, remove_legacy_core_type_columns.sql
    #   - remove_legacy_columns.sql, recreate_pattern_master_table.sql
    #   - split_core_box_s7_f4_column.sql
    # JS Scripts:
    #   - add_allowed_pages.js, run_migration.js, test_products.js
    #   - create_auth_table.js (incorrect/obsolete script)
    #   - create_icsoft.js (incorrect/obsolete test script)
...
foreach ($file in $filesToDelete) {
    ...
    if (Test-Path $file) {
        ...
        Write-Host "⚠️ File not found (already clean): $file" -ForegroundColor Gray
    }
}
...
foreach ($dir in $dirsToDelete) {
    ...
    if (Test-Path $dir) {
        ...
        Write-Host "⚠️ Directory not found (already clean): $dir" -ForegroundColor Gray
    }
}
...
foreach ($component in $unusedComponentsToDelete) {
    if (Test-Path $component) {
        ...
        Write-Host "📦 Unused component exists (not deleted): $component" -ForegroundColor DarkYellow
    }
    else {
        Write-Host "⚠️ Component not found (already deleted): $component" -ForegroundColor Gray
    }
}
