...
const physicalPropertiesSchema = z.object({
    ...
    PartNo: z.coerce.string().max(50).optional().nullable(),
...
const whatsappContactSchema = z.object({
    ...
    PhoneNumber: z.string()
        ...
        .regex(/^\d+$/, 'Phone number must contain only digits (include country code, e.g., 919876543210)'),
    Department: z.string().max(100).optional().nullable(),
}).passthrough();

const whatsappSendTextSchema = z.object({
    ContactIds: z.array(z.coerce.number().int().positive()).min(1, 'At least one contact required'),
    Message: z.string().max(4096, 'Message must be less than 4096 characters').optional(),
}).passthrough();

const whatsappSendReportSchema = z.object({
    ReportId: z.coerce.number().int().positive('Report ID is required'),
    ContactIds: z.array(z.coerce.number().int().positive()).min(1, 'At least one contact required'),
    Format: z.enum(['pdf', 'excel'], { errorMap: () => ({ message: 'Format must be pdf or excel' }) }),
}).passthrough();
