...
router.get('/recovery', cacheMiddleware('ar-recovery', 300), async (req, res) => {
    try {
        ...
        if (fromDate && toDate) {
            const fromMM = fromDate.replace(/-/g, '').substring(0, 6);
            const toMM = toDate.replace(/-/g, '').substring(0, 6);
            if (!/^\d{6}$/.test(fromMM) || !/^\d{6}$/.test(toMM)) {
                return res.status(400).json({ error: 'Invalid date format. Expected YYYY-MM or YYYY-MM-DD.' });
            }
            request.input('fromMM', sql.VarChar(6), fromMM);
            request.input('toMM', sql.VarChar(6), toMM);
