    ...
    try {
        ...
        let query = `
            SELECT
                ...
                g.GradeName
            FROM Product p
            inner join RawMaterial r on r.RawMatID=p.RawMatID
            LEFT JOIN Grade g ON p.Gid = g.GradeId
            WHERE Iscreateproduct = 'Y' AND GrnTypeId not in (176)
        `;
        const request = req.db.request();

        if (search) {
            query += ' AND (p.InternalPartNo LIKE @search OR p.ProdName LIKE @search)';
            request.input('search', sql.NVarChar, '%' + search + '%');
        }
