...
const REPORT_QUERY_TIMEOUT_MS = 30000;

function validateSelectOnly(sqlQuery) {
    const normalized = sqlQuery.replace(/\/\*[\s\S]*?\*\//g, '').replace(/--.*$/gm, '').trim();
    const firstKeyword = normalized.split(/\s+/)[0].toUpperCase();
    if (firstKeyword !== 'SELECT' && firstKeyword !== 'WITH') {
        return false;
    }
    const forbidden = /\b(INSERT|UPDATE|DELETE|DROP|ALTER|CREATE|TRUNCATE|EXEC|EXECUTE|MERGE|GRANT|REVOKE|DENY|xp_|sp_)\b/i;
...
router.post('/:id/preview', async (req, res) => {
    try {
        ...
        if (!validateSelectOnly(report.SqlQuery)) {
            return res.status(400).json({ error: 'Only SELECT/WITH queries are allowed in reports' });
        }

        const previewRequest = targetPool.request();
        previewRequest.timeout = REPORT_QUERY_TIMEOUT_MS;
        const dataResult = await previewRequest.query(report.SqlQuery);
        ...
        res.json({
            ...
            data: dataResult.recordset.slice(0, 100)
        });
    } catch (error) {
...
router.post('/:id/execute', async (req, res) => {
    ...
    try {
        ...
        const execRequest = targetPool.request();
        execRequest.timeout = REPORT_QUERY_TIMEOUT_MS;
        const dataResult = await execRequest.query(report.SqlQuery);
