...
// GET /production-dashboard/family - Product Family and Segment Family data
router.get('/family', cacheMiddleware('production-family', 300), async (req, res) => {
    try {
        const { fromDate, toDate } = req.query;

        const { fromDateValue, toDateValue } = parseDateRange(fromDate, toDate);

        logger.info(`[Production Dashboard - Family] Fetching data from ${fromDateValue.toISOString()} to ${toDateValue.toISOString()}`);
        ...
        logger.info(`[Production Dashboard - Family] Filtering for months: ${monthsInRange.join(', ')}`);
        ...
        // Query the ProductionDashboardFamily view
        const result = await request.query(`
            SELECT ProductType, SegmentType, Month, Pouredweight, OkWeight, RejWeight
            FROM [${DB.POOLS.ERP}].dbo.ProductionDashboardFamily
            ${monthConditions ? `WHERE (${monthConditions})` : ''}
        ...
        logger.error('Error fetching production dashboard family data:', err);
        res.status(500).json({
            error: 'Failed to fetch production dashboard family data',
            ...(process.env.NODE_ENV !== 'production' && { details: err.message })
        });
    }
});
