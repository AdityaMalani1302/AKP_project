...
router.get('/melting-data', cacheMiddleware('production-melting', 300), async (req, res) => {
    try {
        ...
        console.log(`[Production Dashboard - Melting] Fetching data from ${fromDateValue.toISOString()} to ${toDateValue.toISOString()}`);
        ...
        console.log(`[Production Dashboard - Melting] Filtering for months: ${monthsInRange.join(', ')}`);
        ...
        const result = await request.query(`
            ...
            FROM [IcSoftVer3].dbo.MeltingDashboard
        ...
        console.error('Error fetching melting dashboard data:', err);
        res.status(500).json({
...
router.get('/production-data', cacheMiddleware('production-data', 300), async (req, res) => {
    try {
        ...
        console.log(`[Production Dashboard - Production] Fetching data from ${fromDateValue.toISOString()} to ${toDateValue.toISOString()}`);
        ...
        console.log(`[Production Dashboard - Production] Filtering for months: ${monthsInRange.join(', ')}`);
        ...
        const result = await request.query(`
            ...
            FROM [IcSoftVer3].dbo.ProductionDashboard
            ${monthConditions ? `WHERE (${monthConditions})` : ''}
