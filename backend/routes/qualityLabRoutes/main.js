...
const { getCellValue, parseExcelDate } = require('../../utils/excelImport');
...
router.use(requirePage('quality-lab'));
...
router.get('/physical-properties', async (req, res) => {
    try {
        ...
        let query = `
            SELECT 
                lp.Id, lp.Date, lp.HeatNo, lp.Grade, lp.PartNo,
                p.InternalPartNo AS PartNoDisplay,
                lp.[UTS N/mm²], lp.[Yield Stress N/mm²], lp.[Elongation %], lp.[Impact In Joule(J)]
            FROM Lab_PhysicalProperties lp
            LEFT JOIN Product p ON CASE WHEN ISNUMERIC(lp.PartNo) = 1 THEN CAST(lp.PartNo AS INT) ELSE NULL END = p.ProdId
            WHERE 1=1
        `;
        ...
        if (!hasSpecificFilters) {
            query += ` AND MONTH(lp.Date) = MONTH(GETDATE()) AND YEAR(lp.Date) = YEAR(GETDATE())`;
        }
        if (startDate) {
            query += ` AND lp.Date >= @startDate`;
            request.input('startDate', sql.DateTime, new Date(startDate));
        }
        if (endDate) {
            query += ` AND lp.Date < DATEADD(day, 1, @endDate)`;
            request.input('endDate', sql.DateTime, new Date(endDate));
        }

        if (heatNo && heatNo.trim() !== '') {
            query += ` AND LTRIM(RTRIM(CAST(lp.HeatNo AS NVARCHAR(MAX)))) = @heatNo`;
            request.input('heatNo', sql.NVarChar, heatNo.trim());
        }
        if (partNo && partNo.trim() !== '') {
            query += ` AND lp.PartNo = @partNo`;
            request.input('partNo', sql.NVarChar, partNo.trim());
        }
        if (search) {
            query += ` AND (lp.HeatNo LIKE @search OR lp.Grade LIKE @search OR p.InternalPartNo LIKE @search)`;
...
router.get('/microstructure', async (req, res) => {
    try {
        ...
        let query = `
            SELECT 
                lm.ID, lm.Date, lm.HeatNo, lm.Grade, lm.PartNo,
                p.InternalPartNo AS PartNoDisplay,
                lm.Nodularity, lm.Graphitetype, lm.NodularityCount, lm.GraphiteSize,
                lm.Pearlite, lm.Ferrite, lm.Carbide, lm.CastingHardness
            FROM Lab_Micro lm
            LEFT JOIN Product p ON CASE WHEN ISNUMERIC(lm.PartNo) = 1 THEN CAST(lm.PartNo AS INT) ELSE NULL END = p.ProdId
            WHERE 1=1
        `;

        const hasSpecificFilters = (heatNo && heatNo.trim() !== '') || (partNo && partNo.trim() !== '') || search || allTime || startDate || endDate;
        if (!hasSpecificFilters) {
            query += ` AND MONTH(lm.Date) = MONTH(GETDATE()) AND YEAR(lm.Date) = YEAR(GETDATE())`;
        }
        if (startDate) {
            query += ` AND lm.Date >= @startDate`;
            request.input('startDate', sql.DateTime, new Date(startDate));
        }
        if (endDate) {
            query += ` AND lm.Date < DATEADD(day, 1, @endDate)`;
            request.input('endDate', sql.DateTime, new Date(endDate));
        }
        
        if (heatNo && heatNo.trim() !== '') {
            query += ` AND LTRIM(RTRIM(CAST(lm.HeatNo AS NVARCHAR(MAX)))) = @heatNo`;
            request.input('heatNo', sql.NVarChar, heatNo.trim());
        }
        if (partNo && partNo.trim() !== '') {
            query += ` AND lm.PartNo = @partNo`;
            request.input('partNo', sql.NVarChar, partNo.trim());
        }
        if (search) {
            query += ` AND (lm.HeatNo LIKE @search OR lm.Grade LIKE @search OR p.InternalPartNo LIKE @search)`;
            request.input('search', sql.NVarChar, `%${search}%`);
        }
        query += ` ORDER BY lm.Date DESC`;
...
router.get('/sand', async (req, res) => {
    try {
        ...
        let query = `
            SELECT 
                ls.Id, ls.Date, ls.Shift, ls.InspectionTime, ls.HeatNo, ls.PartNo,
                p.InternalPartNo AS PartNoDisplay,
                ls.PartName,
                ls.[Moisture In %], ls.[Compactability In %], ls.[Permeability In No],
                ls.[Green Compression Strength], ls.[Return Sand Temp],
                ls.[TOTAL CLAY 11.0 - 14.50%], ls.[ACTIVE CLAY 7.0 - 9.0%], ls.[DEAD CLAY 3.0 - 4.50%],
                ls.[VOLATILE MATTER 2.30 - 3.50%], ls.[LOSS ON IGNITION 4.0 - 7.0%], ls.[AFS No  45 - 55]
            FROM Lab_Sand ls
            LEFT JOIN Product p ON CASE WHEN ISNUMERIC(ls.PartNo) = 1 THEN CAST(ls.PartNo AS INT) ELSE NULL END = p.ProdId
            WHERE 1=1
        `;

        const hasSpecificFilters = (heatNo && heatNo.trim() !== '') || (partNo && partNo.trim() !== '') || search || allTime || startDate || endDate;
        if (!hasSpecificFilters) {
            query += ` AND MONTH(ls.Date) = MONTH(GETDATE()) AND YEAR(ls.Date) = YEAR(GETDATE())`;
        }
        if (startDate) {
            const parsedStartDate = parseDateString(startDate);
            if (parsedStartDate) {
                query += ` AND ls.Date >= @startDate`;
        ...
        if (endDate) {
            ...
            if (parsedEndDate) {
                query += ` AND ls.Date < DATEADD(day, 1, @endDate)`;
                request.input('endDate', sql.DateTime, parsedEndDate);
            }
        }
        
        if (heatNo && heatNo.trim() !== '') {
            query += ` AND LTRIM(RTRIM(CAST(ls.HeatNo AS NVARCHAR(MAX)))) = @heatNo`;
            request.input('heatNo', sql.NVarChar, heatNo.trim());
        }
        if (partNo && partNo.trim() !== '') {
            query += ` AND ls.PartNo = @partNo`;
            request.input('partNo', sql.NVarChar, partNo.trim());
        }
        if (search) {
            query += ` AND (ls.HeatNo LIKE @search OR p.InternalPartNo LIKE @search OR ls.PartName LIKE @search)`;
            request.input('search', sql.NVarChar, `%${search}%`);
        }
        query += ` ORDER BY ls.Date DESC, ls.InspectionTime ASC`;
...
router.get('/chemistry', async (req, res) => {
    try {
        ...
        let query = `
            SELECT 
                ls.Id, ls.Date, ls.HeatNo, ls.Grade, ls.PartNo,
                p.InternalPartNo AS PartNoDisplay,
                ls.CE, ls.C, ls.Si, ls.Mn, ls.P, ls.S,
                ls.Cu, ls.Cr, ls.Al, ls.Pb, ls.Sn, ls.Ti, ls.Mg, ls.Mo,
                ls.MeltingSupervisor, ls.LabSupervisor
            FROM Lab_Spectro ls
            LEFT JOIN Product p ON CASE WHEN ISNUMERIC(ls.PartNo) = 1 THEN CAST(ls.PartNo AS INT) ELSE NULL END = p.ProdId
            WHERE 1=1
        `;
        ...
        if (search) {
            query += ` AND (ls.HeatNo LIKE @search OR ls.Grade LIKE @search OR p.InternalPartNo LIKE @search)`;
            request.input('search', sql.NVarChar, `%${search}%`);
        }
...
router.get('/mould-hardness', async (req, res) => {
    try {
        ...
        let query = `
            SELECT 
                mh.Id, mh.Date, mh.HeatNo, mh.PartNo,
                p.InternalPartNo AS PartNoDisplay,
                mh.BoxNo1, mh.BoxNo2, mh.BoxNo3, mh.BoxNo4, mh.BoxNo5,
                mh.BoxNo6, mh.BoxNo7, mh.BoxNo8, mh.BoxNo9, mh.BoxNo10,
                mh.BoxNo11, mh.BoxNo12, mh.BoxNo13, mh.BoxNo14, mh.BoxNo15,
                mh.BoxNo16, mh.BoxNo17, mh.BoxNo18, mh.BoxNo19, mh.BoxNo20,
                mh.BoxNo21, mh.BoxNo22, mh.BoxNo23, mh.BoxNo24, mh.BoxNo25
            FROM MouldHardness mh
            LEFT JOIN Product p ON CASE WHEN ISNUMERIC(mh.PartNo) = 1 THEN CAST(mh.PartNo AS INT) ELSE NULL END = p.ProdId
            WHERE 1=1
        `;

        const hasSpecificFilters = (heatNo && heatNo.trim() !== '') || (partNo && partNo.trim() !== '') || search || allTime || startDate || endDate;
        if (!hasSpecificFilters) {
            query += ` AND MONTH(mh.Date) = MONTH(GETDATE()) AND YEAR(mh.Date) = YEAR(GETDATE())`;
        }
        if (startDate) {
            query += ` AND mh.Date >= @startDate`;
            request.input('startDate', sql.DateTime, new Date(startDate));
        }
        if (endDate) {
            query += ` AND mh.Date < DATEADD(day, 1, @endDate)`;
            request.input('endDate', sql.DateTime, new Date(endDate));
        }

        if (heatNo && heatNo.trim() !== '') {
            query += ` AND LTRIM(RTRIM(CAST(mh.HeatNo AS NVARCHAR(MAX)))) = @heatNo`;
            request.input('heatNo', sql.NVarChar, heatNo.trim());
        }
        if (partNo && partNo.trim() !== '') {
            query += ` AND mh.PartNo = @partNo`;
            request.input('partNo', sql.NVarChar, partNo.trim());
        }
        if (search) {
            query += ` AND (mh.HeatNo LIKE @search OR p.InternalPartNo LIKE @search)`;
            request.input('search', sql.NVarChar, `%${search}%`);
        }
...
router.get('/sand/process-capability', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to calculate process capability analysis' });
    }
});
