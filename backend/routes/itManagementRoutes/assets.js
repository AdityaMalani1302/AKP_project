...
router.put('/system-users/:id', validateBody(itSystemUserSchema), async (req, res) => {
    const { id } = req.params;
    const { AssetId, AssignedUser, SystemName, IPAddress, AssetOwner, Descriptions, IssueDate } = req.body;
...
router.put('/device-repaired/:id', validateBody(itDeviceRepairSchema), async (req, res) => {
    const { id } = req.params;
    const { AssetId, RepairDate, IssueDescription, ActionTaken, RepairedBy, RepairCost, VendorName, Status, Notes } = req.body;
...
router.put('/:id', validateBody(itAssetSchema), async (req, res) => {
    const { id } = req.params;
    const {
        AssetTagNumber, AssetName, AssetType, Category, Manufacturer, Model,
        SerialNumber, Hostname, Location, Processor, RAM, StorageTypeCapacity,
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const depsCheck = await req.db.request()
            .input('assetId', sql.Int, assetId)
            .query(`
                SELECT
                    (SELECT COUNT(*) FROM IT_SystemUserDetails WHERE AssetId = @assetId) AS userCount,
                    (SELECT COUNT(*) FROM IT_SoftwareList WHERE InstalledOnAssetId = @assetId) AS softwareCount,
                    (SELECT COUNT(*) FROM IT_DeviceRepairedHistory WHERE AssetId = @assetId) AS repairCount
            `);
        const { userCount, softwareCount, repairCount } = depsCheck.recordset[0];
        const reasons = [];
        if (userCount > 0) reasons.push(`${userCount} system user(s)`);
        if (softwareCount > 0) reasons.push(`${softwareCount} software record(s)`);
        if (repairCount > 0) reasons.push(`${repairCount} repair history record(s)`);

        if (reasons.length > 0) {
            return res.status(409).json({
                error: `Cannot delete this asset — it has ${reasons.join(', ')} linked to it. Remove them first.`
            });
        }

        const request = new sql.Request(req.db);
        request.input('id', sql.Int, assetId);
