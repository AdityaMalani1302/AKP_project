...
router.put('/:id', validateBody(itComplaintSchema), async (req, res) => {
    const { id } = req.params;
    const {
        EmployeeName, Department, ContactNumber, DeviceName, IssueType,
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const resolvedCheck = await req.db.request()
            .input('ticketId', sql.Int, ticketId)
            .query('SELECT TOP 1 ResolvedId FROM IT_Resolved WHERE TicketId = @ticketId');
        if (resolvedCheck.recordset.length > 0) {
            return res.status(409).json({
                error: 'Cannot delete this complaint — it has resolution records. Remove the resolution first.'
            });
        }
...
router.put('/resolved/:id', validateBody(itResolutionSchema), async (req, res) => {
    const { id } = req.params;
    const { TicketId, Date: resolvedDate, ShortIssueTitle, Description } = req.body;
