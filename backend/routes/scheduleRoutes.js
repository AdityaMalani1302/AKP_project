...
router.post('/', validateBody(scheduleSchema), async (req, res) => {
    try {
        const { ReportId, ScheduleName, Frequency, DayOfWeek, DayOfMonth, TimeOfDay, WhatsAppEnabled, ReportFormat } = req.body;
        ...
        const result = await pool.request()
            ...
            .input('WhatsAppEnabled', WhatsAppEnabled ? 1 : 0)
            .input('ReportFormat', ReportFormat || 'pdf')
            .query(`
                INSERT INTO ReportSchedules (ReportId, ScheduleName, Frequency, DayOfWeek, DayOfMonth, TimeOfDay, WhatsAppEnabled, ReportFormat)
                OUTPUT INSERTED.ScheduleId
                VALUES (@ReportId, @ScheduleName, @Frequency, @DayOfWeek, @DayOfMonth, @TimeOfDay, @WhatsAppEnabled, @ReportFormat)
            `);
