...
router.get('/data', cacheMiddleware('sales-data', 120), async (req, res) => {
    try {
        ...
        console.log(`[Sales Dashboard] Fetching data from ${fromDateValue.toISOString()} to ${toDateValue.toISOString()}`);

        const monthsInRange = buildMonthsInRange(fromDateValue, toDateValue);

        console.log(`[Sales Dashboard] Filtering for months: ${monthsInRange.join(', ')}`);
        ...
        try {
            result = await request.query(`
                ...
                FROM [IcSoftVer3].dbo.SalesDashboard
                ${monthConditions ? `WHERE (${monthConditions})` : ''}
                ORDER BY Month
            `);
        } catch (queryErr) {
            console.log('[Sales Dashboard] CountryName column not found, using fallback query');
            result = await request.query(`
