...
router.get('/', requirePage('lab-master'), drawingMasterController.getAllDrawings);

// GET /drawing-master/drawing-numbers - Get all drawing numbers for dropdown (cached 5 min)
router.get('/drawing-numbers', requirePage('lab-master'), cacheMiddleware('drawing-numbers', 300), drawingMasterController.getDrawingNumbers);

// GET /drawing-master/attachment/:attachmentId - Serve a specific attachment file
router.get('/attachment/:attachmentId', requirePage('lab-master'), drawingMasterController.getAttachment);

// GET /drawing-master/by-drawing/:drawingNo - Get record by Drawing No
// NOTE: This must come BEFORE /:id to avoid conflict
router.get('/by-drawing/:drawingNo', requirePage('lab-master'), drawingMasterController.getDrawingByNumber);

// GET /drawing-master/:id - Get single drawing master record
router.get('/:id', requirePage('lab-master'), drawingMasterController.getDrawingById);

// POST /drawing-master - Create new drawing master record (with optional multi-file upload)
router.post('/', requirePage('lab-master'), upload.array('attachments', 10), drawingMasterController.createDrawing);
...
// PUT /drawing-master/:id - Update existing drawing master record (with optional multi-file upload)
router.put('/:id', requirePage('lab-master'), upload.array('attachments', 10), drawingMasterController.updateDrawing);

// DELETE /drawing-master/attachment/:attachmentId - Delete a single attachment
router.delete('/attachment/:attachmentId', requirePage('lab-master'), drawingMasterController.deleteAttachment);
