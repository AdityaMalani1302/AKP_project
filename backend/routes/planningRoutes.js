...
const { validateBody, planningMasterSchema, planningEntrySchema } = require('../utils/validators');
...
router.put('/planning-entry/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('Error updating planning entry:', err);
...
router.post('/sleeve-requirement', async (req, res) => {
    const { planDate, shift, entries } = req.body;

    if (!planDate || !shift || !entries || entries.length === 0) {
        return res.status(400).json({ error: 'Plan Date, Shift, and entries are required' });
    }
    ...
    try {
        ...
        console.error('Error saving sleeve requirements:', err);
...
router.get('/sleeve-requirement/records', async (req, res) => {
    try {
        ...
        console.error('Error fetching sleeve requirement records:', err);
...
router.get('/sleeve-requirement/print-data', async (req, res) => {
    ...
    try {
        ...
        console.error('Error fetching sleeve print data:', err);
