...
const { parseExcelFile, getCellValue, extractHeaders, buildColumnMapping: buildColumnMappingShared, getMappedValue } = require('../utils/excelImport');
...
const buildColumnMapping = (headers) => {
    return buildColumnMappingShared(headers, LAB_COLUMN_MAP, { '9': 'BaseChe_C', '10': 'BaseChe_Si' });
};
...
importExcel: async (req, res) => {
    try {
        ...
        // Step 2: Extract and map headers (auto-detects header row)
        const { headers, headerRowNum } = extractHeaders(worksheet);
        logger.info(`[Lab Import] Header row: ${headerRowNum}, Detected Headers: ${JSON.stringify(headers)}`);
        
        const columnMap = buildColumnMapping(headers);

        // Step 3: Process rows (data starts immediately after the header row)
        let successCount = 0;
        let errorCount = 0;
        let skippedCount = 0;
        const errors = [];
