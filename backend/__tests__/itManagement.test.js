...
describe('IT Management Routes', () => {
  ...
  describe('Assets CRUD', () => {
    ...
    test('DELETE /assets/:id should delete asset', async () => {
      mockQueryFn
        .mockResolvedValueOnce({ recordset: [{ userCount: 0, softwareCount: 0, repairCount: 0 }] })
  ...
  describe('System Users CRUD', () => {
    ...
    test('POST /system-users should create new user', async () => {
      mockQueryFn.mockResolvedValue({ recordset: [{ Id: 1 }] });

      const response = await request(app)
        .post('/it-management/system-users')
        .send({ AssetId: 1, AssignedUser: 'newuser', SystemName: 'TestSystem' });
  ...
  describe('Device Repairs CRUD', () => {
    test('GET /device-repaired should return all repairs', async () => {
      mockQueryFn.mockResolvedValue({
        recordset: [{ RepairId: 1, Issue: 'Broken screen' }],
      });

      const response = await request(app).get('/it-management/device-repaired');

      expect(response.status).toBe(200);
    });

    test('POST /device-repaired should create new repair', async () => {
      mockQueryFn.mockResolvedValue({ recordset: [{ RepairId: 1 }] });

      const response = await request(app)
        .post('/it-management/device-repaired')
        .send({ AssetId: 1, IssueDescription: 'New repair' });
